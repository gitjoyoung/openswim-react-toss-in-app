...
for (const pool of pools) {
  ...
  const fields = [];
  if (needsSchedule && source.freeSwim.length) {
    pool.free_swim = source.freeSwim;
    fields.push("free_swim");
  }
  if (needsPrice && source.price != null) {
    pool.price_free_swim = source.price;
  ...
  changes.push({
    ...
    sourceUpdatedAt: source.sourceLastModified,
    sourcePage: source.sourceUrl,
    primarySource: source.sourceUrl,
  });
}
